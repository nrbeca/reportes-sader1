...
<style>
    ...
    section[data-testid="stSidebar"] { background: linear-gradient(180deg, #D3D3D3 0%, #C0C0C0 100%); }
    section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] label, section[data-testid="stSidebar"] span { color: #333 !important; }
    section[data-testid="stSidebar"] h3 { color: #9B2247 !important; }
    ...
    try:
            ...
            with tab2:
                ...
                if not resultados_ur:
                    ...
                    with col_der:
                        ...
                        st.dataframe(df_cap.style.format({'Original': '${:,.2f}', 'Mod. Anual': '${:,.2f}', 'Mod. Periodo': '${:,.2f}', 'Ejercido': '${:,.2f}', 'Disponible': '${:,.2f}', '% Avance': '{:.2f}%'}), use_container_width=True, hide_index=True)
                        
                        # Top 5 partidas
                        st.markdown("#### Cinco partidas con mayor disponible")
                        ...
                        if partidas_ur:
                            ...
                            for p in partidas_ur[:5]:
                                ...
                                part_data.append({'Partida': p['Partida'], 'Programa': p['Programa'], 'Denom. Programa': p.get('Denom_Programa', ''), 'Disponible': p['Disponible'], '% del Total': pct_r})
        ...
        # ====================================================================
            ...
            with tab2:
                ...
                st.info("Este dashboard requiere el archivo de Cuenta Publica 2024. Cuando lo tengas, lo incorporamos.")
